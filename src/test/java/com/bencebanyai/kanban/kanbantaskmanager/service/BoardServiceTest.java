...
class BoardServiceTest {
    ...
    void setUp() {
        ...
        boardDto = new BoardDto(1L, "Test Board", "Description", Instant.now(), Instant.now(), false, 1L);
    }
    ...
    void createBoard_WithValidRequest_ShouldReturnBoardDto() {
        ...
        when(boardMapper.boardToDto(any(Board.class))).thenReturn(boardDto);

        BoardDto result = boardService.createBoard(request, userEmail);

        assertNotNull(result);
        assertEquals("Test Board", result.name());
        verify(userRepository).findByEmail(userEmail);
        verify(boardRepository).save(any(Board.class));
        verify(boardMapper).boardToDto(board);
    }
    ...
    void getBoardById_WithExistingBoard_ShouldReturnBoardDto() {
        ...
        when(boardMapper.boardToDto(board)).thenReturn(boardDto);
    ...
    void getBoardById_WithNonExistingBoard_ShouldThrowException() {
        ...
        verify(boardMapper, never()).boardToDto(any());
    }
    ...
    void updateBoard_WithValidRequest_ShouldReturnUpdatedBoardDto() {
        ...
        BoardDto result = boardService.updateBoard(1L, request);
    ...
    void getUserBoards_ShouldReturnListOfBoardDtos() {
        ...
        List<BoardDto> results = boardService.getUserBoards(userEmail);
