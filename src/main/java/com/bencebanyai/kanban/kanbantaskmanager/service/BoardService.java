...
public class BoardService {
    ...
    @Transactional
    public BoardResponse createBoard(CreateBoardRequest request, String userEmail) {
        User owner =
                userRepository
    ...
    @Transactional(readOnly = true)
    public List<BoardResponse> getUserBoards(String userEmail) {
        User owner =
                userRepository
                        .findByEmail(userEmail)
                        .orElseThrow(
        ...
        return boardRepository.findByOwnerIdAndIsArchivedFalse(owner.getId()).stream()
                .map(boardMapper::boardToResponse)
                .toList();
    }
}
